...
import { Building2, ShoppingCart, Bus, Plane } from "lucide-react";

// --- Data for our City ---
const cityData = [
    { id: 1, position: [-3, 1, 2], scale: [1, 2, 1], name: "Corporate Park", icon: Building2, insight: "High concentration of professionals. Recommend `Ad8Social` LinkedIn campaigns & `8DigiAd` screens in office lobbies." },
    { id: 2, position: [0, 0.75, 2], scale: [1, 1.5, 1], name: "City Centre", icon: ShoppingCart, insight: "Peak footfall on weekends. Recommend `Ad8OOH` on main avenues & `Ad8Mobi` geo-targeted offers." },
    { id: 3, position: [3, 1.25, 2], scale: [1, 2.5, 1], name: "Transport Hub", icon: Bus, insight: "Constant flow of commuters. Recommend `Ad8Radio` spots during rush hour & ads on public transport." },
    { id: 4, position: [-2, 0.5, -2], scale: [2, 1, 1], name: "Airport", icon: Plane, insight: "High-value audience with long dwell times. Recommend premium `8DigiAd` placements in terminals." },
];

// --- Road Component ---
const Road = ({ start, end, width = 0.3 }: { start: number[], end: number[], width?: number }) => {
...
// --- 3D Components ---
const InteractiveBuilding = ({ data, onPointerOver, onPointerOut, onClick }: any) => {
    ...
    const texture = useMemo(() => createBuildingTexture(buildingTypes[data.id as keyof typeof buildingTypes]), [data.id]);
    const targetColor = isHovered ? new THREE.Color("#a855f7") : new THREE.Color("#ffffff");
    ...
    return (
        <mesh
            ...
            scale={data.scale}
            onPointerOver={(e) => { e.stopPropagation(); setIsHovered(true); onPointerOver(data); }}
            onPointerOut={() => { setIsHovered(false); onPointerOut(null); }}
...
const CityScene = ({ onBuildingHover, onBuildingClick }: any) => {
    return (
        <>
            <ambientLight intensity={0.5} />
            <directionalLight position={[10, 10, 5]} intensity={1.5} />
            <directionalLight position={[-10, -10, -5]} intensity={0.3} />
            <pointLight position={[0, 5, 0]} intensity={1} distance={15} color="#a855f7" />
      ...
      <div className="mx-auto max-w-7xl px-6 lg:px-8 grid lg:grid-cols-2 gap-16 items-center">
        ...
        >
          ...
          <p className="mt-4 text-lg sm:text-xl text-white/70">
            Explore a simulated city district. Hover over buildings to identify hyperlocal opportunities and click to get an instant AI-powered recommendation.
          </p>
            ...
            >
                {hoveredBuilding ? (
                    <div>
                        ...
                        <p className="mt-2 text-white/80">Click to reveal Aura's strategic insight for this location.</p>
                    </div>
                ) : (
                    <p className="text-white/60">Hover over a building in the city to learn more.</p>
                )}
            </motion.div>
          </AnimatePresence>
        </motion.div>
        ...
        <div className="w-full h-[500px] lg:h-[600px] cursor-grab active:cursor-grabbing rounded-lg border border-white/10">
            ...
            <Suspense fallback={null}>
                <CityScene onBuildingHover={setHoveredBuilding} onBuildingClick={setActiveInsight} />
            </Suspense>
            <OrbitControls autoRotate autoRotateSpeed={0.5} maxPolarAngle={Math.PI / 2.2} minPolarAngle={Math.PI / 4} />
            </Canvas>
        </div>
      </div>
                ...
                >
                    <div className="flex justify-between items-start">
                        <div className="flex items-center gap-4">
                            ...
                            <h3 className="font-bold text-2xl font-serif">{activeInsight.name}</h3>
                        </div>
                        <button onClick={() => setActiveInsight(null)} className="text-2xl text-white/50 hover:text-white">&times;</button>
                    </div>
                    <p className="mt-6 text-lg text-white/80">{activeInsight.insight}</p>
                </motion.div>
            </motion.div>
        )}
      </AnimatePresence>
